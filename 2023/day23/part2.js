export const task = async () => {
	...
	while (queue.length) {
		...
		if ((row > 0) && ('.^'.includes(lines[row - 1][col])))
			next(row - 1, col, step, path);

		if ((row < lines.length - 1) && ('.v'.includes(lines[row + 1][col])))
			next(row + 1, col, step,  path);

		if ((col > 0) && ('.<'.includes(lines[row][col - 1])))
			next(row, col - 1, step, path);

		if ((col < lines[0].length - 1) && ('.>'.includes(lines[row][col + 1])))
			next(row, col + 1, step, path);

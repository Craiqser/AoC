...
const checkSign = (lines: string[], lineNum: number, idxStart: number, idxEnd: number): boolean => {
	...
	if (isSymbol(lines[lineNum][searchFrom])) {
		return true;
	}
	if (isSymbol(lines[lineNum][searchTo])) {
		return true;
	}
	if ((lineNum > 0) && findSymbol(lines[lineNum - 1], searchFrom, searchTo)) {
		return true;
	}
	if ((lineNum < (lines.length - 1)) && findSymbol(lines[lineNum + 1], searchFrom, searchTo)) {
		return true;
	}
	return false;
};

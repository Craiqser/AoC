...
const checkLine = (line: string, j: number) => {
	...
	if (isDigit(line[j - 1])) {
		for (let k = j - 1; (k >= 0) && isDigit(line[k]); k--) {
			idxStart = k;
		}
	...
	if (isDigit(line[j + 1])) {
		...
		for (let k = j; (k < line.length) && isDigit(line[k]); k++) {
			digit = digit + line[k];
		}
...
const findNumbers = (lines: string[], i: number, j: number): number => {
	...
	for (let k = j - 1; (k >= 0) && isDigit(lines[i][k]); k--) {
		digit = lines[i][k] + digit;
	}
	...
	for (let k = j + 1; (k < lines[i].length) && isDigit(lines[i][k]); k++) {
		digit = digit + lines[i][k];
	}
	...
	if (digits.length === 2) {
		res = parseInt(digits[0]) * parseInt(digits[1]);
	}
